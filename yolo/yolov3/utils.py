...
danger_items = {"person", "hammer", "scissors", "knife", "drill", "wrench", "stairs", "nail"}
send_enable = True
send_cnt=0
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            try:
                ...
                print(label)
                if  NUM_CLASS[class_ind] in danger_items:
                    item = NUM_CLASS[class_ind]
                    gpio.output(buzzer, gpio.HIGH)
                    send_cnt += 1
                    print(send_cnt)
                    image_path = '/tmp/foo.jpg'
                    cv2.imwrite(image_path, image)
                    mqtt_send(image_path, item)
                    if send_cnt % 3 == 0 :
                        mqtt_send(original_image_path, 'train')
                    if send_cnt > 1000:
                        send_cnt -= 1000
                else: 
                    gpio.output(buzzer, gpio.LOW)
...
def mqtt_send(image_path, item) :
    with open(image_path, "rb") as img_file:
        my_string = base64.b64encode(img_file.read())
    client = mqtt.Client()
    client.connect("10.121.190.108",1883,60)
    byteArray = bytes(my_string)
    cmd = 'mosquitto_pub -h 10.121.190.108 -t "topic/' + str(item) + '" -m "{}"'.format(byteArray)
    cmd = re.sub("-m \"b'","-m \"",cmd)
    cmd = cmd[:-2]
    cmd += '"'
    #print(cmd)
    print(item)
